...
- if a small portion of table is being affected it results in row-lock, but if the porion be big enough the lock will be row-level instead of a high number of row-level blocks.
### Primitive functions
````SQL
-- Create table
CREATE TABLE table(
